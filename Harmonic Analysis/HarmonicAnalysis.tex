...
\begin{defn}[Schwartz space][Space!Schwartz] \label{def:SchwartzSpace} The Schwartz space is defined as \[ \schwartz = \set{ f ∈ C^∞(ℝ^n) \st \sup_{x ∈ ℝ^n} \abs{x^N ∂_x^α f(x)} < ∞ } \] for any $N ∈ ℕ$ and any multiindex $α ∈ ℕ^n$.
\end{defn}
...
\begin{theorem}[Fourier\IS inversion] \label{thm:FourierInversion} Let $f ∈ \schwartz$ and $\hat{f}$ its Fourier transform. Then, the inversion formula holds: \( f(x) = \int_{ℝ^n} \hat{f}(ξ) e^{2π\imath x · ξ} \dif ξ \)
\end{theorem}
...
\begin{lemma}[Hadamard\IS three lines lemma] \label{lem:Hadamard3Lines} Let $F$ be a complex analytic function on the strip $S ≝ \set{x ∈ ℂ \tq 0 < \Re z < 1}$ extending continuously and boundedly to the closure $\adh{S}$. Asssume the bounds \begin{align*}
...
In order to apply the \nref{lem:Hadamard3Lines} we need to bound $\abs{F(z)}$ when $\Re z = 0, 1$. In both cases, we can see by the Hölder inequality that
...
\begin{defn}[Hilbert\IS transform]  \label{def:HilbertTrans}  Let $f ∈ C_0^∞(ℝ)$ and $ε > 0$. The, we can define \( \label{eq:HilbertTransEpsilon} H_ε f(x) ≝ \int_{-∞}^{-ε} \frac{f(x-y)}{y} \dif y + \int_ε^∞ \frac{f(x-y)}{y} \dif y = \int_ε^∞ \frac{f(x-y) - f(x + y)}{y} \dif y \) and \[ Hf(x) ≝ \lim_{ε \to 0} H_ε f(x) \]
\end{defn}
...
\begin{defn}[Calderón-Zygmund kernel][Kernel\IS Calderón-Zygmund] \label{def:CalderonZygmundKernel} A function $K ∈ C^∞(ℝ^n \setminus\set{0})$ is called a Calderón-Zygmund kernel if $K$ satisfies
\begin{enumerate}
\item \label{prp:CZAss1} $\abs{K(x)} ≤ B \abs{x}^{-n} \quad ∀x ∈ ℝ^n, \, B > 0 $
\item \label{prp:CZAss2} The \concept{Hörmander\IS condition}: \[ \int_{\abs{x} > \abs{2y}} (K(x) - K(x-y)) \dif x ≤ CB \quad ∀x ∈ ℝ^n, \, B > 0,\, y ≠ 0 \]
...
The second condition, which is a little bit convoluted, can be ensured by a more elementary condition, namely that $K$ must be differentiable on $ℝ^n \minuszero$ and $\abs{∇K(x)} ≤ B\abs{x}^{-n-1}$ for all $x ≠ 0$. Kernels with this condition are called \concept[Calderón-Zygmund\IS strong kernel]{strong Calderón-Zygmund kernels}, and the following lemma ensures that we indeed have the implication.

\begin{lemma} \label{lem:HormanderCond} The strong Calderón-Zygmund condition for a \nref{def:CalderonZygmundKernel} implies the Hörmander condition \ref{prp:CZAss2} \[ \int_{\abs{x} > 2\abs{y}} (K(x) - K(x-y)) \dif x  ≤ CB \quad ∀ y ≠ 0\]
\end{lemma}
...
\begin{defn}[Calderón-Zygmund operator][Operator\IS Calderón-Zygmund] \label{def:CalderonZygmundOperator} Given a \nref{def:CalderonZygmundKernel} $K$, we can define the Calderón-Zygmund operator for $f ∈ \schwartz$ as \[ (Tf) (x) ≝ \lim_{ε \to 0+} \int_{\abs{x - y} ≥ ε} K(x -y) f(y) \dif y \]
\end{defn}
...
\begin{theorem}[Calderón-Zygmund\IS theorem] \label{thm:CalderonZygmund} Let $f ∈ L^1(ℝ^n)$ and fix $λ > 0$. Then we can write \[ f = g + b \] wth $\abs{g} ≤ λ$ almost everywhere and $b = \sum_{Q ∈ \mathcal{Q}} \ind_Q f$ with $\mathcal{Q}$ a disjoint collection of cubes $Q ⊂ ℝ^n$ and such that \[ λ < \fint_Q \abs{f(x)} \dif μ < 2^n λ \] where $μ$ is the measure on $ℝ^n$.
...
\begin{defn}[Hardy-Littlewood\IS maximal function] \label{def:HardyLittlewood} For $f ∈ \espLloc$ we define its Hardy-Littlewood maximal function as \[ Mf(x) ≝ \sup_{B \st x ∈ B} \fint_B \abs{f}\] with the supremum running over all balls containing $x$.
\end{defn}
...
\begin{lemma} \label{lem:MaximalApproxId} Let $\set{ρ_ε}_{ε > 0}$ be a family of \nlref{def:ApproximateIdentity} functions. Then we have \[ M_1 f(x) ≤ C Mf(x) \] for a suitable constant $C$. Moreover, if $K ∈ C^1(ℝ^n)$ is radial $K(x) = h(\abs{x})$ and decays with $h' < 0$ and further we have $\abs{h'(r)} ≤ Cr^{-n-2}$ then we also have \[ \sup_{ε > 0} \abs{K_ε * f(x)} ≤ CMf(x) \]
\end{lemma}
...
\begin{defn}[BMO\IS function] \label{def:BMOFunction} Let $f ∈ \espLloc$. Then we say that $f$ is a BMO function if we have \[ \norm{f^\sharp}_{L^∞(ℝ^n)} < ∞ \] where \[ f^\sharp(x) ≝ \sup_{x ∈ B} \fint_B \abs{f(y) - f_B} \dif y \] with $f_B = \fint_B f$ the average of $f$ over $B$. We then write the norm as \[ \norm{f}_\bmo ≝ \norm{f^\sharp}_{L^∞(ℝ^n)} \]
\end{defn}

It's important to see that CZ operators send $L^∞ ∩ L^2$ functions to $\bmo$ space.

\begin{theorem} \label{thm:BoundBMOOper} Let $T$ be a \nref{def:CalderonZygmundOperator} and $f ∈ L^∞ ∩ L^2$. Then we have \[ \norm{Tf}_\bmo ≤ CB \norm{f}_∞ \]
\end{theorem}
...
\begin{theorem}[John-Nirenberg\IS theorem] \label{thm:JohnNirenberg} Let $f ∈ \bmo(ℝ^n)$, and in particular $f ∈ \espLloc$. Then, for some absolute positive positive constants $c_n, C_n$ we have \[ \abs{\set{x ∈ Q \st \abs{f(x) - f_Q} > λ}} ≤ C\abs{Q} e^{-c\frac{λ}{\norm{f}_\bmo}}\] for any cube $Q ⊂ ℝ^n$.
\end{theorem}
...
\begin{defn}[Fourier multiplier] Given $f ∈ \schwartz$ we can define a Fourier multipler oeprator associated to $m(ξ)$ as  \[  Tf ≝ \inv{\fourier}\left(m(ξ) \hat{f}(ξ)\right)\]
\end{defn}
...
\begin{theorem}[Mikhlin\IS theorem] \label{thm:Mikhlin} If $\appl{m}{ℝ^n \minuszero}{ℂ}$ satisfies \[ \abs{∂^\vec{γ }m(ξ)} ≤ B \abs{ξ}^{-\abs{\vec{γ}}} \quad \abs{\vec{γ}} ∈ [0, n+2] \qquad ξ ≠ 0 \], then the Fourier multiplier introduced by $m$ is $L^p$ bounded with \[ \norm{ \inv{\fourier}(m(ξ) \hat{f}(ξ))}_{L^p} ≤ C_{n,p} B\norm{f}_{L^p} \] with $1 < p < ∞$.
\end{theorem}
