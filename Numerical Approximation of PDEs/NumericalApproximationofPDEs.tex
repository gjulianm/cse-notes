...
\[ W^{h,p} (Ω) ≝ \set{ u ∈ L^p(Ω) \tq ∂^αu ∈ L^p(Ω) \; ∀\abs{α} ≤ k} \] allowing distributional derivatives. This is Banach with the norm \[\norm{f}_{W^{h,p}} = \left(\sum_{\abs{α} ≤ h} \norm{∂^αu}_{L^p}^p\right)^{\sfrac{1}{p}} \] with the special case for $p = ∞$ \[ \norm{f}_{W^{h,∞}} = \sup_{\abs{α} ≤ h} \norm{∂^α u}_{L^∞} \]

The special case is with $p = 2$, named as $H^k(Ω) = W^{k,2}(Ω)$, is a Hilbert space with the inner product given by \[ \pesc{f,g}_{H^k(Ω)} ≝ \sum_{\abs{α} ≤ k} \int_Ω ∂^α f · ∂^α g \dif x \], which clearly induces the norm defined previously.

Nice property: embeddings: $H^{k+1} ⊂ H^k(Ω)$ and $H^0 (Ω) = L^2(Ω)$. For notational convenience, we can define the $H^k$ seminorm as simply by \[ \abs{f}_{H^k(Ω)}^2 = \sum_{\abs{α} = h} \norm{∂^αf}_{L^2(Ω)}\], and thus \[ \norm{f}_{H^k(Ω)}^2 = \sum_{m=0}^k \abs{f}^2_{H^m(Ω)} \]

Regularity of the functions depend on the dimension. Question for us, if we take $Ω = \bola_r(0) ⊂ ℝ^2$, show that $f(x,y) = \abs{\ln \left(\frac{1}{\sqrt{x^2 + y^2}}\right)^a}$ is in $H^1$ for $0 < a < \frac{1}{2}$ but is not continous.
