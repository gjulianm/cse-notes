...
\begin{defn}[Galerkin Least Squares] This method is given by the stabilization terms \( \label{eq:ODE:GLS} \begin{aligned}
...
\begin{itemize}
	\item \concept{Forward (explicit) Euler method} With $θ = 0$, first order method with respect to $Δt$.
	\item \concept{Backward (implicit) Euler method} With $θ = 1$, again first order.
...
This is the motivation for the \concept{Method\IS of characteristics}, in which we will solve a differential equation along each characteristic curve. We can easily give an a-priori estimate under the assumptions that $a$ is differentiable with \[ a_0 - \frac{1}{2} \dod{a}{x} ≥ μ_0 > 0 \quad ∀x ∈ [α, β]\]
